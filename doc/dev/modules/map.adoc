...
Specialized entity with a position, activation radius, activation height and default active behavior by distance.

.config
[horizontal]
pos:: `{x,y,z,radius(optional),height(optional)}` (must be floats)
