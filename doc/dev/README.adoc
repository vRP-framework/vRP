...
To not conflict with other extensions, make sure the name of the properties and methods have a very specific name or prefix.
...
vRP provides basic methods to store custom binary data in the database. A string key is associated to a binary value (a Lua string). The key is a `VARCHAR(100)` and the value a `BLOB`.
...
CAUTION: vRP core tables should not be modified for the same reason about core files. New tables and queries should be created if the data system is not powerful enough.
...
vRP 2 extensively uses asynchronous tasks in a transparent way using coroutines, some functions may not return immediately.
...
The content of `server_vrp.lua` can now use vRP and the API.
