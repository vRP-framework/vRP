...
User inherits from all extensions sub-class User.
...
.Events
[horizontal]
`extensionLoad(extension)`:: called when an extension is loaded, passing the extension instance (can be used to initialize with another extension when loaded before)
...
`playerDeath()`:: called owhen the player dies
