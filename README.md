...
Homes are closed interiors allocated to players when they want to go inside their home, it means that if no slots are availables, you can't enter to your home. Slots get released when everyone moves out, however if a player dies, crashes or disconnects inside, the slot will not close itself, only "eject all" commande will close the slot. So it's possible that all slots are locked after a while, restarting the server will fix the issue. 
...
To call the server-side API functions, you need the vRP interface.
...
You can also do it client-side, the API is the as same as the TUNNEL CLIENT APIs (copy and add the vrp/client/Proxy.lua first to your resources directory).
...
The inventory is autosaved and as the wallet it gets empty upon death.
...
By following this way, processing and harvesting are limited by the work units.
...
For static areas, please configure the file `cfg/item_transformers.lua`, the transformers will be automatically added.
...
The main menu is generated by using an event, this is useful to add special choices if needed.
...
Now if you want to use the same teleport function in another resource:
