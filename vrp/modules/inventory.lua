...
-- Define the player inventory module
local Inventory = class("Inventory", vRP.Extension)

-- Define the Inventory User subclass
...
local function e_chest_remove(menu) -- Unload chest
...
function Inventory.User:openChest(id, max_weight, cb_close, cb_in, cb_out)
  if vRP.EXT.Inventory.chests[id] then
...
local function menu_inventory_item(self)
  local function m_give(menu)
    ...
    local nuser = vRP.users_by_source[vRP.EXT.Base.remote.getNearestPlayer(user.source, 10)]
    ...
    local amount = tonumber(user:prompt(lang.inventory.give.prompt({ user:getItemAmount(fullid) }), ""))
    if not amount or amount <= 0 then
      vRP.EXT.Base.remote._notify(user.source, lang.common.invalid_value())
      return
    end

    if not nuser:tryGiveItem(fullid, amount, true) then
      vRP.EXT.Base.remote._notify(user.source, lang.inventory.full())
      return
    end

    if not user:tryTakeItem(fullid, amount, true) then
      vRP.EXT.Base.remote._notify(user.source, lang.common.invalid_value())
      return
    end
  ...
  local function m_trash(menu)
    ...
    local fullid = menu.data.fullid
    local citem = self:computeItem(fullid)
    local amount = tonumber(user:prompt(lang.inventory.trash.prompt({ user:getItemAmount(fullid) }), ""))
    ...
    if not user:tryTakeItem(fullid, amount, nil, true) then
      vRP.EXT.Base.remote._notify(user.source, lang.common.invalid_value())
      return
    end
...
local function menu_chest_take(self)
  local function m_take(menu, fullid)
    ...
    local amount = tonumber(user:prompt(lang.inventory.chest.take.prompt({ i_amount }), ""))
	
	-- Check if the menu is still active
    local currentMenu = user:getMenu()
    if not currentMenu or currentMenu.name ~= "chest.take" then
      vRP.EXT.Base.remote._notify(user.source, "You are no longer interacting with the chest.")
      return
    end
    ...
    if user:tryGiveItem(fullid, amount) then
      ...
      if menu.data.cb_out then
        menu.data.cb_out(menu.data.id, fullid, amount)
      end
...
local function menu_chest_put(self)
  local function m_put(menu, fullid)
    ...
    local amount = tonumber(user:prompt(lang.inventory.chest.put.prompt({ i_amount }), ""))

	-- Check if the menu is still active
    local currentMenu = user:getMenu()
    if not currentMenu or currentMenu.name ~= "chest.put" then
      vRP.EXT.Base.remote._notify(user.source, "You are no longer interacting with the chest.")
      return
    end
	
    if not amount or amount < 0 or amount > i_amount or not user:tryTakeItem(fullid, amount, true) then
      vRP.EXT.Base.remote._notify(user.source, lang.common.invalid_value())
      return
    end
    ...
    if menu.data.cb_in then
      menu.data.cb_in(menu.data.id, fullid, amount)
    end
...
local function menu_chest(self)
  local function m_take(menu)
    local smenu = menu.user:openMenu("chest.take", menu.data)
  ...
  local function m_put(menu)
    local smenu = menu.user:openMenu("chest.put", menu.data)
    menu:listen("remove", function(menu)
      menu.user:closeMenu(smenu)
    end)
  end
...
local function menu_admin_users_user(self)
  ...
  vRP.EXT.GUI:registerMenuBuilder("admin.users.user", function(menu)
    ...
    if tuser and user:hasPermission("player.giveitem") then
...
function Inventory:__construct()
  ...
  local function fperm_item(user, params)
    if #params == 3 then -- decompose item.operator
      ...
      if fop == "<" and amount < parseInt(string.sub(op,2,string.len(op))) then  -- less (item.<x)
        return true
      elseif fop == ">" and amount > parseInt(string.sub(op,2,string.len(op))) then -- greater (item.>x)
        return true
      elseif amount == parseInt(string.sub(op,1,string.len(op))) then -- equal (item.x)
        return true
      end
    end
  end
  ...
  if not citem then -- compute
    ...
    if item then
      ...
      citem = {
        ...
        description = desc,
        weight = weight,
        menu_builder = item.menu_builder,
        args = args
      }
