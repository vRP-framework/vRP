...
local function menu_vehicle(self)
  ...
  local function m_trunk(menu)
    ...
    -- Retrieve trunk distance
    local trunkDistance = self.remote.getNearestOwnedVehicleTrunk(user.source, 1.2) -- Specify the desired radius
	
    if trunkDistance then
        -- Open the trunk
        self.remote.vc_openDoor(user.source, model, 5)

        local smenu = user:openChest(chestid, max_weight, function()
            self.remote.vc_closeDoor(user.source, model, 5)
        end)
		...
		vRP.EXT.Base.remote._notify(user.source, ("The vehicle trunk is too far away."))
    end
  end
  ...
  local function m_asktrunk(menu)
    ...
    if nuser then
      ...
      if nuser:request(lang.vehicle.asktrunk.request(),15) then -- request accepted, open trunk
        local model = self.remote.getNearestOwnedVehicle(nuser.source,2)
        if model then
          ...
          Citizen.CreateThread(function()
            while running do
              Citizen.Wait(5000)
              local check_model = self.remote.getNearestOwnedVehicle(user.source, 2)
              if model ~= check_model then
                user:closeMenu(menu)
              end
            end
